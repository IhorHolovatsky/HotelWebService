...
public class RoomDAO implements IRoomDAO {
    ...
    public List<Room> GetAvailableRooms(Date startDate, Date endDate){
        Session session = sessionFactory.getCurrentSession();
        Query query = session.createQuery("select distinct r from Room as r left join r.RoomBookings br where (br.StartDate not between CONVERT(datetime, :startDate , 103) and CONVERT(datetime, :endDate, 103)) and (br.EndDate not between CONVERT(datetime, :startDate , 103) and CONVERT(datetime, :endDate, 103)) or br.StartDate is null");
