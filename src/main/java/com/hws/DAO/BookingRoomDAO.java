...
public class BookingRoomDAO implements IBookingRoomDAO {
    ...
    public List<BookingRoom> GetBookings(Date startDate, Date endDate){
        Session session = sessionFactory.getCurrentSession();
        Query query = session.createQuery("select br from BookingRoom br where (:startDate between br.StartDate and br.EndDate) or (:endDate between br.StartDate and br.EndDate) or (:startDate < br.StartDate and :endDate > br.EndDate)");
